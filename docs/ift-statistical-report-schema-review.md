...
- [ ] **OpenAPI regeneration** so the published request bodies use the IFT field vocabulary (the future UI consumes the typed client). *(The `crudPaths(...)` helper in `src/utils/openapi.js` derives request bodies from the validation schema, so the published spec already reflects the new field names; re-run the frontend `pnpm run gen:api` step before starting UI work.)*
