...
async function generateInvoicePdf(invoiceId, { locale = 'en' } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const footerText = branding.footer_legal
      ? `${branding.footer_legal} — Generated by ${product.displayName} — ${new Date().toISOString()}`
      : `Generated by ${product.displayName} — ${new Date().toISOString()}`;
...
async function generateCreditNotePdf(creditNoteId, { locale = 'en' } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(7).font('Helvetica').fillColor(COLORS.muted)
      .text(`Generated by ${product.displayName} — ${new Date().toISOString()}`, PAGE_MARGIN, 740, { align: 'center', width: doc.page.width - PAGE_MARGIN * 2 });
...
async function generateClientLedgerPdf(clientId, { locale = 'en', from = null, to = null, orgId = null } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // ---- Footer ----
    doc.fontSize(7).font('Helvetica').fillColor(COLORS.muted)
      .text(`Generated by ${product.displayName} — ${new Date().toISOString()}`, PAGE_MARGIN, 750, { align: 'center', width: doc.page.width - PAGE_MARGIN * 2 });
